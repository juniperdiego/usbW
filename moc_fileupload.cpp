/****************************************************************************
** Meta object code from reading C++ file 'fileupload.h'
**
** Created: Sat Jul 12 10:14:04 2014
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "fileupload.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'fileupload.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_FileUpload[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      19,   12,   11,   11, 0x05,

 // slots: signature, parameters, type, tag, flags
      42,   36,   11,   11, 0x0a,
      77,   11,   11,   11, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_FileUpload[] = {
    "FileUpload\0\0bState\0SetUpState(bool)\0"
    "Reply\0UpFinishSingleFile(QNetworkReply*)\0"
    "HandleError(QNetworkReply::NetworkError)\0"
};

const QMetaObject FileUpload::staticMetaObject = {
    { &QThread::staticMetaObject, qt_meta_stringdata_FileUpload,
      qt_meta_data_FileUpload, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &FileUpload::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *FileUpload::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *FileUpload::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_FileUpload))
        return static_cast<void*>(const_cast< FileUpload*>(this));
    return QThread::qt_metacast(_clname);
}

int FileUpload::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QThread::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: SetUpState((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 1: UpFinishSingleFile((*reinterpret_cast< QNetworkReply*(*)>(_a[1]))); break;
        case 2: HandleError((*reinterpret_cast< QNetworkReply::NetworkError(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void FileUpload::SetUpState(bool _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
